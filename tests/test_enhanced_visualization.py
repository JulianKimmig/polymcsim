...
def branched_polymer_config() -> SimulationInput:
    """Configuration for a branched polymer system."""
    return SimulationInput(
        monomers=[
            MonomerDef(
                ...
                count=50,
                molar_mass=150.0,
                sites=[SiteDef(type="A", status="ACTIVE")] * 3  # Trifunctional
            ),
            MonomerDef(
                name="B2", 
                count=75,
                molar_mass=100.0,
                sites=[SiteDef(type="B", status="ACTIVE")] * 2  # Bifunctional
            ),
        ],
        reactions={
...
def test_conversion_analysis(branched_polymer_config, plot_path: Path):
    ...
    edge_data = [(i, j, t) for (i, j), t in zip(graph.edges(), np.linspace(0, 1, len(graph.edges())))]
    fig = plot_conversion_analysis(
        metadata,
        edge_data=edge_data,

...
def block_copolymer_config() -> SimulationInput:
    """Configuration for a block copolymer (e.g., AAAA-BBBB)."""
    return SimulationInput(
        monomers=[
            MonomerDef(name="Initiator", count=10, sites=[SiteDef(type="I", status="ACTIVE")]),
        ...
        reactions={
            ...
            frozenset(["RadicalA", "RadicalA"]): ReactionSchema(
                site1_final_status="CONSUMED", site2_final_status="CONSUMED", rate=0.1
            ),
            frozenset(["RadicalB", "RadicalB"]): ReactionSchema(
                site1_final_status="CONSUMED", site2_final_status="CONSUMED", rate=0.1
            ),
            frozenset(["RadicalA", "RadicalB"]): ReactionSchema(
                site1_final_status="CONSUMED", site2_final_status="CONSUMED", rate=0.1
            ),
        },
        params=SimParams(max_reactions=180, random_seed=42, name="block_copolymer"),
    )

def test_block_copolymer_formation(block_copolymer_config: SimulationInput, plot_path: Path):
    ...
    types_in_polymer = {data["monomer_type"] for _, data in largest_polymer.nodes(data=True)}
    ...
    a_nodes = [n for n, d in largest_polymer.nodes(data=True) if d['monomer_type'] == 'MonomerA']
    for node in a_nodes:
        b_neighbors = 0
...
def random_copolymer_config() -> SimulationInput:
    """Configuration for a random copolymer."""
    return SimulationInput(
        monomers=[
            MonomerDef(name="Initiator", count=10, sites=[SiteDef(type="I", status="ACTIVE")]),
            MonomerDef(name="MonomerA", count=100, sites=[SiteDef(type="A_in", status="DORMANT"), SiteDef(type="A_out", status="DORMANT")]),
            MonomerDef(name="MonomerB", count=100, sites=[SiteDef(type="B_in", status="DORMANT"), SiteDef(type="B_out", status="DORMANT")]),
        ],
        reactions={
            # Initiation
            frozenset(["I", "A_in"]): ReactionSchema(site1_final_status="CONSUMED", site2_final_status="CONSUMED", activation_map={"A_out": "Radical"}, rate=1.0),
            frozenset(["I", "B_in"]): ReactionSchema(site1_final_status="CONSUMED", site2_final_status="CONSUMED", activation_map={"B_out": "Radical"}, rate=1.0),
            # Propagation
            frozenset(["Radical", "A_in"]): ReactionSchema(site1_final_status="CONSUMED", site2_final_status="CONSUMED", activation_map={"A_out": "Radical"}, rate=5.0),
            frozenset(["Radical", "B_in"]): ReactionSchema(site1_final_status="CONSUMED", site2_final_status="CONSUMED", activation_map={"B_out": "Radical"}, rate=5.0),
        },
        params=SimParams(max_reactions=150, random_seed=43, name="random_copolymer"),
    )


def test_random_copolymer_formation(random_copolymer_config: SimulationInput, plot_path: Path):
...
def alternating_copolymer_config() -> SimulationInput:
    """Configuration for an alternating copolymer (ABABAB)."""
    return SimulationInput(
        monomers=[
            MonomerDef(name="MonomerA", count=100, sites=[SiteDef(type="A", status="ACTIVE"), SiteDef(type="A", status="ACTIVE")]),
            MonomerDef(name="MonomerB", count=100, sites=[SiteDef(type="B", status="ACTIVE"), SiteDef(type="B", status="ACTIVE")]),
        ],
        reactions={
            frozenset(["A", "B"]): ReactionSchema(site1_final_status="CONSUMED", site2_final_status="CONSUMED", rate=10.0),
            # No A-A or B-B reactions allowed
        },
        params=SimParams(max_reactions=150, random_seed=44, name="alternating_copolymer"),
    )


def test_alternating_copolymer_formation(alternating_copolymer_config: SimulationInput, plot_path: Path):

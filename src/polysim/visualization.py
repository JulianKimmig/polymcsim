...
def plot_molecular_weight_distribution(
    ...
    if not molar_masses:
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(0.5, 0.5, "No polymer chains formed.", 
    ...
    n, bins_edges, _ = ax.hist(molar_masses, bins=bins, weights=weights, 
                               alpha=0.75, color='darkseagreen', 
                               edgecolor='black', label='Number distribution')
    ...
    ax2 = ax.twinx()
    ax2.plot(bin_centers, weight_fractions, 'r-', linewidth=2, 
             label='Weight distribution', alpha=0.8)
...
def plot_conversion_analysis(
    ...
    max_conversion_param = metadata.get('config', {}).get('params', {}).get('max_conversion', 1.0)
    ...
    if edge_data and len(edge_data) > 0:
        ...
        total_monomers = metadata.get('config', {}).get('total_monomers', 
                                     reactions_completed * 2)  # fallback estimate
        conversions = np.arange(1, len(times) + 1) * 2 / total_monomers
        
        # Create subplots
        ax1 = plt.subplot(2, 2, 1)
        ax1.plot(times, conversions, 'b-', linewidth=2)
        ax1.axhline(y=max_conversion_param, color='r', linestyle='--', 
...
def plot_branching_analysis(
    ...
    if polymer_degrees:
        ...
        branch_points = counts[unique_degrees > 2].sum() if any(unique_degrees > 2) else 0
        
        total_units = sum(counts)
        branching_freq = branch_points / total_units if total_units > 0 else 0
        
        stats_text = (
            f'Terminal units: {terminal_units} ({terminal_units/total_units*100:.1f}%)\n'
            f'Linear units: {linear_units} ({linear_units/total_units*100:.1f}%)\n'
            f'Branch points: {branch_points} ({branch_points/total_units*100:.1f}%)\n'
            f'Branching frequency: {branching_freq:.3f}'
        )
...
def create_analysis_dashboard(
    ...
    if components and len(components[0]) > 1:
        ...
        node_colors = []
        monomer_types = list(set(nx.get_node_attributes(largest_polymer, 'monomer_type').values()))
        ...
        nx.draw(largest_polymer, pos, node_color=node_colors, 
                node_size=50, with_labels=False, ax=ax1)
    ...
    chain_lengths = [len(c) for c in components if len(c) > 1]
    if chain_lengths:
        ax2.hist(chain_lengths, bins='auto', alpha=0.75, color='cornflowerblue', edgecolor='black')
    ...
    if molar_masses:
        ax3.hist(molar_masses, bins=30, alpha=0.75, color='darkseagreen', edgecolor='black')
        ...
        ax3.text(0.95, 0.95, f'PDI = {pdi:.3f}', transform=ax3.transAxes,
                verticalalignment='top', horizontalalignment='right',
    ...
    ax5.bar(sizes[:20], counts[:20], color='mediumpurple', edgecolor='black', alpha=0.8)  # Limit to first 20
